...
def test_default_dictionary_has_correct_length() -> None:
    result = get_dictionary(DictionaryPath.DEFAULT.value)
...
def test_default_dictionary_has_expected_content() -> None:
    ...
    assert result[1000].definitions[0] == 'a. <i>strongly adverse</i> (andstreym ørlög); Sighvatr var heldr ~ um eptirmálin, <i>hard to come to terms with</i>.'
